...
// MARK: - Shared Helpers: Span

@usableFromInline
package func _nkWithSpanPair<T, R>(
    _ a: borrowing Span<T>,
    _ b: borrowing Span<T>,
    _ body: (UnsafePointer<T>, UnsafePointer<T>, Int) -> R
) -> R? {
    guard !a.isEmpty && a.count == b.count else { return nil }
    return a.withUnsafeBufferPointer { lhs in
        b.withUnsafeBufferPointer { rhs in
            body(lhs.baseAddress!, rhs.baseAddress!, a.count)
        }
    }
}

@usableFromInline
package func _nkWithSpanPairRebound<E, T, R>(
    _ a: borrowing Span<E>,
    _ b: borrowing Span<E>,
    to: T.Type,
    _ body: (UnsafePointer<T>, UnsafePointer<T>, Int) -> R
) -> R? {
    _nkWithSpanPair(a, b) { lhsPtr, rhsPtr, count in
        let lhsRebound = UnsafeRawPointer(lhsPtr).assumingMemoryBound(to: T.self)
        let rhsRebound = UnsafeRawPointer(rhsPtr).assumingMemoryBound(to: T.self)
        return body(lhsRebound, rhsRebound, count)
    }
