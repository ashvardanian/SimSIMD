...
error_stats_t test_reduce_moments(typename input_type_::reduce_moments_kernel_t kernel,
    ...
    auto buffer = make_vector<input_type_>(dense_dimensions);
    for (auto start = test_start_time(); within_time_budget(start);) {
        fill_random(generator, buffer);
        typename sum_type_::raw_t sum;
        typename sumsq_type_::raw_t sumsq;
        kernel(buffer.raw_values_data(), dense_dimensions, sizeof(typename input_type_::raw_t), &sum, &sumsq);
        typename sum_type_::raw_t ref_sum;
        typename sumsq_type_::raw_t ref_sumsq;
        reference(buffer.raw_values_data(), dense_dimensions, sizeof(typename input_type_::raw_t), &ref_sum,
                  &ref_sumsq);
...
error_stats_t test_reduce_minmax(typename input_type_::reduce_minmax_kernel_t kernel,
    ...
    for (auto start = test_start_time(); within_time_budget(start);) {
        ...
        kernel(buffer.raw_values_data(), dense_dimensions, sizeof(typename input_type_::raw_t), &min_val, &min_idx,
               &max_val, &max_idx);
        typename output_type_::raw_t ref_min, ref_max;
        nk_size_t ref_min_idx, ref_max_idx;
        reference(buffer.raw_values_data(), dense_dimensions, sizeof(typename input_type_::raw_t), &ref_min,
