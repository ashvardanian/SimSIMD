...
#if defined(NK_DEFINED_LINUX_) && NK_TARGET_ARM32_
#include <sys/auxv.h> // `getauxval`, `AT_HWCAP`
#endif
...
NK_PUBLIC nk_capability_t nk_capabilities_arm32_(void) {
#if defined(NK_DEFINED_LINUX_)
    unsigned long hwcap = getauxval(AT_HWCAP);
    // AArch32 HWCAP bits from the Linux kernel:
    //   bit 12 (0x1000)    — NEON
    //   bit 22 (0x400000)  — FPHP   (half-precision VFP)
    //   bit 23 (0x800000)  — ASIMDHP (half-precision NEON)
    //   bit 24 (0x1000000) — ASIMDDP (dot-product / SDOT)
    //   bit 25 (0x2000000) — ASIMDFHM (FP16 fused multiply-add)
    int supports_neon = (hwcap >> 12) & 1;
    int supports_fp16 = (hwcap >> 22) & 1 & (hwcap >> 23);
    int supports_dp = (hwcap >> 24) & 1;
    int supports_fhm = (hwcap >> 25) & 1;
    return (nk_capability_t)((nk_cap_neon_k * (supports_neon)) |
                             (nk_cap_neonhalf_k * (supports_neon && supports_fp16)) |
                             (nk_cap_neonsdot_k * (supports_neon && supports_dp)) |
                             (nk_cap_neonfhm_k * (supports_neon && supports_fhm)) | (nk_cap_serial_k));
#else
    return nk_cap_serial_k;
#endif
}

#endif // NK_TARGET_ARM32_
...
#elif NK_TARGET_ARM32_
    nk_unused_(capabilities);
    return 1; // AArch32 has no FPCR.EBF to configure
#endif
    nk_unused_(capabilities);
