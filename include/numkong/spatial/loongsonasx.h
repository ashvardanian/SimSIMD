...
NK_PUBLIC void nk_angular_f32_loongsonasx(nk_f32_t const *a, nk_f32_t const *b, nk_size_t n, nk_f64_t *result) {
    ...
    for (; i + 8 <= n; i += 8) {
        ...
        a_sq_f64x4_hi = __lasx_xvfmadd_d(a_high_f64x4, a_high_f64x4, a_sq_f64x4_hi);
        b_sq_f64x4_lo = __lasx_xvfmadd_d(b_low_f64x4, b_low_f64x4, b_sq_f64x4_lo);
        b_sq_f64x4_hi = __lasx_xvfmadd_d(b_high_f64x4, b_high_f64x4, b_sq_f64x4_hi);
    }
    nk_f64_t dot = nk_reduce_add_f64x4_loongsonasx_(__lasx_xvfadd_d(dot_f64x4_lo, dot_f64x4_hi));
    nk_f64_t a_sq = nk_reduce_add_f64x4_loongsonasx_(__lasx_xvfadd_d(a_sq_f64x4_lo, a_sq_f64x4_hi));
    nk_f64_t b_sq = nk_reduce_add_f64x4_loongsonasx_(__lasx_xvfadd_d(b_sq_f64x4_lo, b_sq_f64x4_hi));
    for (; i < n; ++i) {
