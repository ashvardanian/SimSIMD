...
#include "dot.h"     // `_mathkong_partial_load_f16x4_neon` and friends
#include "spatial.h" // `_mathkong_substract_bf16x32_genoa`
...
#define SIMSIMD_MAKE_BILINEAR(name, input_type, accumulator_type, load_and_convert)                                 \
    SIMSIMD_PUBLIC void mathkong_bilinear_##input_type##_##name(                                                    \
        ...
        for (mathkong_size_t i = 0; i != n; ++i) {                                                                  \
            mathkong_##accumulator_type##_t cb_j = 0;                                                               \
            mathkong_##accumulator_type##_t a_i = load_and_convert(a + i);                                          \
            for (mathkong_size_t j = 0; j != n; ++j) {                                                              \
                mathkong_##accumulator_type##_t b_j = load_and_convert(b + j);                                      \
                mathkong_##accumulator_type##_t c_ij = load_and_convert(c + i * n + j);                             \
                cb_j += c_ij * b_j;                                                                                 \
            }                                                                                                       \
            sum += a_i * cb_j;                                                                                      \
        }                                                                                                           \
...
#define SIMSIMD_MAKE_COMPLEX_BILINEAR(name, input_type, accumulator_type, load_and_convert)                 \
    SIMSIMD_PUBLIC void mathkong_bilinear_##input_type##_##name(                                            \
        ...
        mathkong_##accumulator_type##_t sum_imag = 0;                                                       \
        for (mathkong_size_t i = 0; i != n; ++i) {                                                          \
            mathkong_##accumulator_type##_t cb_j_real = 0;                                                  \
            mathkong_##accumulator_type##_t cb_j_imag = 0;                                                  \
            mathkong_##accumulator_type##_t a_i_real = load_and_convert(&(a_pairs + i)->real);              \
            mathkong_##accumulator_type##_t a_i_imag = load_and_convert(&(a_pairs + i)->imag);              \
            for (mathkong_size_t j = 0; j != n; ++j) {                                                      \
                mathkong_##accumulator_type##_t b_j_real = load_and_convert(&(b_pairs + j)->real);          \
                mathkong_##accumulator_type##_t b_j_imag = load_and_convert(&(b_pairs + j)->imag);          \
                mathkong_##accumulator_type##_t c_ij_real = load_and_convert(&(c_pairs + i * n + j)->real); \
                mathkong_##accumulator_type##_t c_ij_imag = load_and_convert(&(c_pairs + i * n + j)->imag); \
        ...
        results[0] = (mathkong_distance_t)sum_real;                                                         \
...
#define SIMSIMD_MAKE_MAHALANOBIS(name, input_type, accumulator_type, load_and_convert)                              \
    SIMSIMD_PUBLIC void mathkong_mahalanobis_##input_type##_##name(                                                 \
        ...
        for (mathkong_size_t i = 0; i != n; ++i) {                                                                  \
            mathkong_##accumulator_type##_t cdiff_j = 0;                                                            \
            mathkong_##accumulator_type##_t diff_i = load_and_convert(a + i) - load_and_convert(b + i);             \
            for (mathkong_size_t j = 0; j != n; ++j) {                                                              \
                mathkong_##accumulator_type##_t diff_j = load_and_convert(a + j) - load_and_convert(b + j);         \
                mathkong_##accumulator_type##_t c_ij = load_and_convert(c + i * n + j);                             \
                cdiff_j += c_ij * diff_j;                                                                           \
            }                                                                                                       \
            sum += diff_i * cdiff_j;                                                                                \
...
SIMSIMD_PUBLIC void mathkong_bilinear_f32_neon(mathkong_f32_t const *a, mathkong_f32_t const *b,
    ...
    for (mathkong_size_t i = 0; i != n; ++i) {
        ...
        for (mathkong_size_t j = 0; j + 4 <= n; j += 4) {
    ...
    mathkong_f64_t sum = vaddvq_f32(sum_vec);
    mathkong_size_t const tail_length = n % 4;
    mathkong_size_t const tail_start = n - tail_length;
    if (tail_length) {
        for (mathkong_size_t i = 0; i != n; ++i) {
            mathkong_f32_t a_i = a[i];
            mathkong_f32_t cb_j = 0;
            for (mathkong_size_t j = tail_start; j != n; ++j) cb_j += b[j] * c[i * n + j];
            sum += a[i] * cb_j;
        }
    }
...
SIMSIMD_PUBLIC void mathkong_mahalanobis_f32_neon(mathkong_f32_t const *a, mathkong_f32_t const *b,
    ...
    if (tail_length) {
        for (mathkong_size_t i = 0; i != n; ++i) {
            mathkong_f32_t diff_i = a[i] - b[i];
            mathkong_f32_t cdiff_j = 0;
            for (mathkong_size_t j = tail_start; j != n; ++j) {
                mathkong_f32_t diff_j = a[j] - b[j];
...
SIMSIMD_PUBLIC void mathkong_bilinear_f32c_neon(mathkong_f32c_t const *a, mathkong_f32c_t const *b,
    ...
    for (mathkong_size_t i = 0; i != n; ++i) {
        mathkong_f32c_t a_i = a[i];
        mathkong_f32c_t cb_j;
        ...
        for (mathkong_size_t j = 0; j + 4 <= n; j += 4) {
            ...
            float32x4x2_t b_vec = vld2q_f32((mathkong_f32_t const *)(b + j));
            float32x4x2_t c_vec = vld2q_f32((mathkong_f32_t const *)(c + i * n + j));
    ...
    if (tail_length) {
        for (mathkong_size_t i = 0; i != n; ++i) {
            mathkong_f32c_t a_i = a[i];
            mathkong_f32c_t cb_j = {0, 0};
            for (mathkong_size_t j = tail_start; j != n; ++j) {
                mathkong_f32c_t b_j = b[j];
                mathkong_f32c_t c_ij = c[i * n + j];
...
SIMSIMD_PUBLIC void mathkong_bilinear_f16_neon(mathkong_f16_t const *a, mathkong_f16_t const *b,
    ...
    for (mathkong_size_t i = 0; i != n; ++i) {
        ...
        float32x4_t cb_j_vec = vdupq_n_f32(0);
        for (mathkong_size_t j = 0; j + 4 <= n; j += 4) {
            float32x4_t b_vec = vcvt_f32_f16(vld1_f16((mathkong_f16_for_arm_simd_t const *)(b + j)));
            float32x4_t c_vec = vcvt_f32_f16(vld1_f16((mathkong_f16_for_arm_simd_t const *)(c + i * n + j)));
            cb_j_vec = vmlaq_f32(cb_j_vec, b_vec, c_vec);
        }
        sum_vec = vmlaq_f32(sum_vec, a_vec, cb_j_vec);
    }
    ...
    if (tail_length) {
        for (mathkong_size_t i = 0; i != n; ++i) {
            mathkong_f32_t a_i = vaddvq_f32(vcvt_f32_f16(_mathkong_partial_load_f16x4_neon(a + i, 1)));
            float32x4_t b_vec = vcvt_f32_f16(_mathkong_partial_load_f16x4_neon(b + tail_start, tail_length));
            float32x4_t c_vec = vcvt_f32_f16(_mathkong_partial_load_f16x4_neon(c + i * n + tail_start, tail_length));
            mathkong_f32_t cb_j = vaddvq_f32(vmulq_f32(b_vec, c_vec));
            sum += a_i * cb_j;
        }
    }
...
SIMSIMD_PUBLIC void mathkong_mahalanobis_f16_neon(mathkong_f16_t const *a, mathkong_f16_t const *b,
    ...
    for (mathkong_size_t i = 0; i != n; ++i) {
        ...
        float32x4_t cdiff_j_vec = vdupq_n_f32(0);
        for (mathkong_size_t j = 0; j + 4 <= n; j += 4) {
            float32x4_t a_j_vec = vcvt_f32_f16(vld1_f16((mathkong_f16_for_arm_simd_t const *)(a + j)));
            float32x4_t b_j_vec = vcvt_f32_f16(vld1_f16((mathkong_f16_for_arm_simd_t const *)(b + j)));
    ...
    if (tail_length) {
        for (mathkong_size_t i = 0; i != n; ++i) {
            mathkong_f32_t a_i = vaddvq_f32(vcvt_f32_f16(_mathkong_partial_load_f16x4_neon(a + i, 1)));
            mathkong_f32_t b_i = vaddvq_f32(vcvt_f32_f16(_mathkong_partial_load_f16x4_neon(b + i, 1)));
            mathkong_f32_t diff_i = a_i - b_i;
            float32x4_t a_j_vec = vcvt_f32_f16(_mathkong_partial_load_f16x4_neon(a + tail_start, tail_length));
            float32x4_t b_j_vec = vcvt_f32_f16(_mathkong_partial_load_f16x4_neon(b + tail_start, tail_length));
            float32x4_t diff_j_vec = vsubq_f32(a_j_vec, b_j_vec);
            float32x4_t c_vec = vcvt_f32_f16(_mathkong_partial_load_f16x4_neon(c + i * n + tail_start, tail_length));
            mathkong_f32_t cdiff_j = vaddvq_f32(vmulq_f32(diff_j_vec, c_vec));
            sum += diff_i * cdiff_j;
        }
    }
...
SIMSIMD_PUBLIC void mathkong_bilinear_f16c_neon(mathkong_f16c_t const *a, mathkong_f16c_t const *b,
    ...
    mathkong_size_t const tail_start = n - tail_length;
    for (mathkong_size_t i = 0; i != n; ++i) {
        mathkong_f32c_t a_i = {mathkong_f16_to_f32(&a[i].real), mathkong_f16_to_f32(&a[i].imag)};
        float16x8_t cb_j_real_vec = vdupq_n_f16(0);
        float16x8_t cb_j_imag_vec = vdupq_n_f16(0);
        for (mathkong_size_t j = 0; j + 8 <= n; j += 8) {
            // Unpack the input arrays into real and imaginary parts:
        ...
        if (tail_length) {
            // Unpack the input arrays into real and imaginary parts:
            float16x8x2_t b_vec = _mathkong_partial_load_f16x8x2_neon(b + tail_start, tail_length);
            float16x8x2_t c_vec = _mathkong_partial_load_f16x8x2_neon(c + i * n + tail_start, tail_length);
            float16x8_t b_real_vec = b_vec.val[0];
            float16x8_t b_imag_vec = b_vec.val[1];
            float16x8_t c_real_vec = c_vec.val[0];
            float16x8_t c_imag_vec = c_vec.val[1];
        ...
        mathkong_f32c_t cb_j;
        cb_j.real = _mathkong_reduce_f16x8_neon(cb_j_real_vec);
        cb_j.imag = _mathkong_reduce_f16x8_neon(cb_j_imag_vec);
        sum_real += a_i.real * cb_j.real - a_i.imag * cb_j.imag;
        sum_imag += a_i.real * cb_j.imag + a_i.imag * cb_j.real;
    }
...
SIMSIMD_PUBLIC void mathkong_bilinear_bf16_neon(mathkong_bf16_t const *a, mathkong_bf16_t const *b,
    ...
    for (mathkong_size_t i = 0; i != n; ++i) {
        ...
        float32x4_t cb_j_vec = vdupq_n_f32(0);
        for (mathkong_size_t j = 0; j + 8 <= n; j += 8) {
            bfloat16x8_t b_vec = vld1q_bf16((mathkong_bf16_for_arm_simd_t const *)(b + j));
            bfloat16x8_t c_vec = vld1q_bf16((mathkong_bf16_for_arm_simd_t const *)(c + i * n + j));
    ...
    if (tail_length) {
        for (mathkong_size_t i = 0; i != n; ++i) {
            mathkong_f32_t a_i;
            mathkong_bf16_to_f32(a + i, &a_i);
            bfloat16x8_t b_vec = _mathkong_partial_load_bf16x8_neon(b + tail_start, tail_length);
            bfloat16x8_t c_vec = _mathkong_partial_load_bf16x8_neon(c + i * n + tail_start, tail_length);
            mathkong_f32_t cb_j = vaddvq_f32(vbfdotq_f32(vdupq_n_f32(0), b_vec, c_vec));
            sum += a_i * cb_j;
        }
    }
...
SIMSIMD_PUBLIC void mathkong_mahalanobis_bf16_neon(mathkong_bf16_t const *a, mathkong_bf16_t const *b,
    ...
    for (mathkong_size_t i = 0; i != n; ++i) {
        ...
        mathkong_bf16_to_f32(a + i, &a_i);
        mathkong_bf16_to_f32(b + i, &b_i);
        float32x4_t diff_i_vec = vdupq_n_f32(a_i - b_i);
        float32x4_t cdiff_j_vec = vdupq_n_f32(0);
        for (mathkong_size_t j = 0; j + 8 <= n; j += 8) {
            bfloat16x8_t a_j_vec = vld1q_bf16((mathkong_bf16_for_arm_simd_t const *)(a + j));
            bfloat16x8_t b_j_vec = vld1q_bf16((mathkong_bf16_for_arm_simd_t const *)(b + j));
    ...
    if (tail_length) {
        for (mathkong_size_t i = 0; i != n; ++i) {
            ...
            mathkong_bf16_to_f32(b + i, &b_i);
            mathkong_f32_t diff_i = a_i - b_i;
            bfloat16x8_t a_j_vec = _mathkong_partial_load_bf16x8_neon(a + tail_start, tail_length);
            bfloat16x8_t b_j_vec = _mathkong_partial_load_bf16x8_neon(b + tail_start, tail_length);
            ...
            bfloat16x8_t c_vec = _mathkong_partial_load_bf16x8_neon(c + i * n + tail_start, tail_length);
            mathkong_f32_t cdiff_j = vaddvq_f32(vbfdotq_f32(vdupq_n_f32(0), diff_j_vec, c_vec));
            sum += diff_i * cdiff_j;
        }
    }
...
SIMSIMD_PUBLIC void mathkong_bilinear_bf16c_neon(mathkong_bf16c_t const *a, mathkong_bf16c_t const *b,
    ...
    mathkong_size_t const tail_start = n - tail_length;
    for (mathkong_size_t i = 0; i != n; ++i) {
        mathkong_f32c_t a_i = {mathkong_bf16_to_f32(&a[i].real), mathkong_bf16_to_f32(&a[i].imag)};
        ...
        if (tail_length) {
            // Unpack the input arrays into real and imaginary parts:
            int16x4x2_t b_vec = _mathkong_partial_load_bf16x4x2_neon(b + tail_start, tail_length);
            int16x4x2_t c_vec = _mathkong_partial_load_bf16x4x2_neon(c + i * n + tail_start, tail_length);
            float32x4_t b_real_vec = vcvt_f32_bf16(vreinterpret_bf16_s16(b_vec.val[0]));
            float32x4_t b_imag_vec = vcvt_f32_bf16(vreinterpret_bf16_s16(b_vec.val[1]));
            float32x4_t c_real_vec = vcvt_f32_bf16(vreinterpret_bf16_s16(c_vec.val[0]));
            float32x4_t c_imag_vec = vcvt_f32_bf16(vreinterpret_bf16_s16(c_vec.val[1]));
...
SIMSIMD_PUBLIC void mathkong_bilinear_f16_haswell(mathkong_f16_t const *a, mathkong_f16_t const *b,
    ...
    if (tail_length) {
        for (mathkong_size_t i = 0; i != n; ++i) {
            mathkong_f32_t a_i = _mm256_cvtss_f32(_mm256_cvtph_ps(_mm_set1_epi16(*(short const *)(a + i))));
            __m256 b_vec = _mathkong_partial_load_f16x8_haswell(b + tail_start, tail_length);
            __m256 c_vec = _mathkong_partial_load_f16x8_haswell(c + i * n + tail_start, tail_length);
            mathkong_f32_t cb_j = _mathkong_reduce_f32x8_haswell(_mm256_mul_ps(b_vec, c_vec));
            sum += a_i * cb_j;
        }
    }
...
SIMSIMD_PUBLIC void mathkong_mahalanobis_f16_haswell(mathkong_f16_t const *a, mathkong_f16_t const *b,
    ...
    if (tail_length) {
        for (mathkong_size_t i = 0; i != n; ++i) {
            mathkong_f32_t diff_i = _mm256_cvtss_f32(_mm256_sub_ps(       //
                _mm256_cvtph_ps(_mm_set1_epi16(*(short const *)(a + i))), //
                _mm256_cvtph_ps(_mm_set1_epi16(*(short const *)(b + i)))));
            __m256 diff_j_vec = _mm256_sub_ps( //
                _mathkong_partial_load_f16x8_haswell(a + tail_start, tail_length),
                _mathkong_partial_load_f16x8_haswell(b + tail_start, tail_length));
            __m256 c_vec = _mathkong_partial_load_f16x8_haswell(c + i * n + tail_start, tail_length);
            mathkong_f32_t cdiff_j = _mathkong_reduce_f32x8_haswell(_mm256_mul_ps(diff_j_vec, c_vec));
            sum += diff_i * cdiff_j;
        }
    }
...
SIMSIMD_PUBLIC void mathkong_bilinear_bf16_haswell(mathkong_bf16_t const *a, mathkong_bf16_t const *b,
    ...
    for (mathkong_size_t i = 0; i != n; ++i) {
        ...
        __m256 a_vec = _mm256_set1_ps(mathkong_bf16_to_f32(a + i));
        __m256 cb_j_vec = _mm256_setzero_ps();
        for (mathkong_size_t j = 0; j + 8 <= n; j += 8) {
            __m256 b_vec = _mathkong_bf16x8_to_f32x8_haswell(_mm_lddqu_si128((__m128i const *)(b + j)));
            __m256 c_vec = _mathkong_bf16x8_to_f32x8_haswell(_mm_lddqu_si128((__m128i const *)(c + i * n + j)));
            cb_j_vec = _mm256_fmadd_ps(b_vec, c_vec, cb_j_vec);
        }
        sum_vec = _mm256_fmadd_ps(a_vec, cb_j_vec, sum_vec);
    }
    ...
    if (tail_length) {
        for (mathkong_size_t i = 0; i != n; ++i) {
            mathkong_f32_t a_i;
            mathkong_bf16_to_f32(a + i, &a_i);
            __m256 b_vec = _mathkong_bf16x8_to_f32x8_haswell( //
                _mathkong_partial_load_bf16x8_haswell(b + tail_start, tail_length));
            __m256 c_vec = _mathkong_bf16x8_to_f32x8_haswell( //
                _mathkong_partial_load_bf16x8_haswell(c + i * n + tail_start, tail_length));
            mathkong_f32_t cb_j = _mathkong_reduce_f32x8_haswell(_mm256_mul_ps(b_vec, c_vec));
            sum += a_i * cb_j;
        }
    }
...
SIMSIMD_PUBLIC void mathkong_mahalanobis_bf16_haswell(mathkong_bf16_t const *a, mathkong_bf16_t const *b,
    ...
    for (mathkong_size_t i = 0; i != n; ++i) {
        __m256 diff_i_vec = _mm256_sub_ps(               //
            _mm256_set1_ps(mathkong_bf16_to_f32(a + i)), //
            _mm256_set1_ps(mathkong_bf16_to_f32(b + i)));
        __m256 cdiff_j_vec = _mm256_setzero_ps();
        for (mathkong_size_t j = 0; j + 8 <= n; j += 8) {
            __m256 diff_j_vec = _mm256_sub_ps(                                                //
                _mathkong_bf16x8_to_f32x8_haswell(_mm_lddqu_si128((__m128i const *)(a + j))), //
                _mathkong_bf16x8_to_f32x8_haswell(_mm_lddqu_si128((__m128i const *)(b + j))));
            __m256 c_vec = _mathkong_bf16x8_to_f32x8_haswell(_mm_lddqu_si128((__m128i const *)(c + i * n + j)));
            cdiff_j_vec = _mm256_fmadd_ps(diff_j_vec, c_vec, cdiff_j_vec);
        }
        sum_vec = _mm256_fmadd_ps(diff_i_vec, cdiff_j_vec, sum_vec);
    ...
    if (tail_length) {
        for (mathkong_size_t i = 0; i != n; ++i) {
            ...
            mathkong_f32_t diff_i = a_i - b_i;
            __m256 diff_j_vec = _mm256_sub_ps( //
                _mathkong_bf16x8_to_f32x8_haswell(_mathkong_partial_load_bf16x8_haswell(a + tail_start, tail_length)),
                _mathkong_bf16x8_to_f32x8_haswell(_mathkong_partial_load_bf16x8_haswell(b + tail_start, tail_length)));
            __m256 c_vec = _mathkong_bf16x8_to_f32x8_haswell(
                _mathkong_partial_load_bf16x8_haswell(c + i * n + tail_start, tail_length));
            mathkong_f32_t cdiff_j = _mathkong_reduce_f32x8_haswell(_mm256_mul_ps(diff_j_vec, c_vec));
            sum += diff_i * cdiff_j;
...
SIMSIMD_PUBLIC void mathkong_bilinear_f32_skylake(mathkong_f32_t const *a, mathkong_f32_t const *b,
    ...
    if (n <= 16) {
        mathkong_bilinear_f32_skylake_under16unrolled(a, b, c, n, result);
        return;
    }
    ...
    mathkong_bilinear_f32_skylake_cycle:
        if (j + 16 <= n) {
        ...
        j += 16;
        if (j < n) goto mathkong_bilinear_f32_skylake_cycle;
...
SIMSIMD_PUBLIC void mathkong_bilinear_f32c_skylake(mathkong_f32c_t const *a, mathkong_f32c_t const *b,
    ...
    for (mathkong_size_t i = 0; i != n; ++i) {
        mathkong_f32_t const a_i_real = a[i].real;
        mathkong_f32_t const a_i_imag = a[i].imag;
        __m512 cb_j_real_vec = _mm512_setzero_ps();
        __m512 cb_j_imag_vec = _mm512_setzero_ps();
        __m512 b_vec, c_vec;
        mathkong_size_t j = 0;

    mathkong_bilinear_f32c_skylake_cycle:
        if (j + 8 <= n) {
            b_vec = _mm512_loadu_ps((mathkong_f32_t const *)(b + j));
            c_vec = _mm512_loadu_ps((mathkong_f32_t const *)(c + i * n + j));
        }
        else {
            b_vec = _mm512_maskz_loadu_ps(tail_mask, (mathkong_f32_t const *)(b + tail_start));
            c_vec = _mm512_maskz_loadu_ps(tail_mask, (mathkong_f32_t const *)(c + i * n + tail_start));
        }
        ...
        j += 8;
        if (j < n) goto mathkong_bilinear_f32c_skylake_cycle;
        ...
        mathkong_f32_t const cb_j_real = _mm512_reduce_add_ps(cb_j_real_vec);
        mathkong_f32_t const cb_j_imag = _mm512_reduce_add_ps(cb_j_imag_vec);
        sum_real += a_i_real * cb_j_real - a_i_imag * cb_j_imag;
        sum_imag += a_i_real * cb_j_imag + a_i_imag * cb_j_real;
    }
...
SIMSIMD_PUBLIC void mathkong_bilinear_f64_skylake(mathkong_f64_t const *a, mathkong_f64_t const *b,
    ...
    if (n <= 8) {
        mathkong_bilinear_f64_skylake_under8unrolled(a, b, c, n, result);
        return;
    }
    ...
    mathkong_bilinear_f64_skylake_cycle:
        if (j + 8 <= n) {
        ...
        j += 8;
        if (j < n) goto mathkong_bilinear_f64_skylake_cycle;
...
SIMSIMD_PUBLIC void mathkong_bilinear_f64c_skylake(mathkong_f64c_t const *a, mathkong_f64c_t const *b,
    ...
    mathkong_f64_t sum_real = 0;
    mathkong_f64_t sum_imag = 0;

    for (mathkong_size_t i = 0; i != n; ++i) {
        mathkong_f64_t const a_i_real = a[i].real;
        mathkong_f64_t const a_i_imag = a[i].imag;
        __m512d cb_j_real_vec = _mm512_setzero_pd();
        __m512d cb_j_imag_vec = _mm512_setzero_pd();
        __m512d b_vec, c_vec;
        mathkong_size_t j = 0;

    mathkong_bilinear_f64c_skylake_cycle:
        if (j + 4 <= n) {
            b_vec = _mm512_loadu_pd((mathkong_f64_t const *)(b + j));
            c_vec = _mm512_loadu_pd((mathkong_f64_t const *)(c + i * n + j));
        }
        else {
            b_vec = _mm512_maskz_loadu_pd(tail_mask, (mathkong_f64_t const *)(b + tail_start));
            c_vec = _mm512_maskz_loadu_pd(tail_mask, (mathkong_f64_t const *)(c + i * n + tail_start));
        }
        // The real part of the product: b.real * c.real - b.imag * c.imag.
        // The subtraction will be performed later with a sign flip.
        ...
        j += 4;
        if (j < n) goto mathkong_bilinear_f64c_skylake_cycle;
        // Flip the sign bit in every second scalar before accumulation:
        cb_j_real_vec = _mm512_castsi512_pd(_mm512_xor_si512(_mm512_castpd_si512(cb_j_real_vec), sign_flip_vec));
        // Horizontal sums are the expensive part of the computation:
        mathkong_f64_t const cb_j_real = _mm512_reduce_add_pd(cb_j_real_vec);
        mathkong_f64_t const cb_j_imag = _mm512_reduce_add_pd(cb_j_imag_vec);
        sum_real += a_i_real * cb_j_real - a_i_imag * cb_j_imag;
        sum_imag += a_i_real * cb_j_imag + a_i_imag * cb_j_real;
    }
...
SIMSIMD_PUBLIC void mathkong_bilinear_bf16_genoa(mathkong_bf16_t const *a, mathkong_bf16_t const *b,
    ...
    for (mathkong_size_t i = 0; i != n; ++i) {
        __m512 a_vec = _mm512_set1_ps(mathkong_bf16_to_f32(a + i));
        __m512 cb_j_vec = _mm512_setzero_ps();
        __m512i b_vec, c_vec;
        mathkong_size_t j = 0;

    mathkong_bilinear_bf16_genoa_cycle:
        if (j + 32 <= n) {
        ...
        j += 32;
        if (j < n) goto mathkong_bilinear_bf16_genoa_cycle;
        sum_vec = _mm512_fmadd_ps(a_vec, cb_j_vec, sum_vec);
    }
...
SIMSIMD_PUBLIC void mathkong_mahalanobis_bf16_genoa(mathkong_bf16_t const *a, mathkong_bf16_t const *b,
    ...
    for (mathkong_size_t i = 0; i != n; ++i) {
        __m512 diff_i_vec = _mm512_set1_ps(mathkong_bf16_to_f32(a + i) - mathkong_bf16_to_f32(b + i));
        __m512 cdiff_j_vec = _mm512_setzero_ps();
        __m512i a_j_vec, b_j_vec, diff_j_vec, c_vec;
        mathkong_size_t j = 0;

        // The nested loop is cleaner to implement with a `goto` in this case:
    mathkong_mahalanobis_bf16_genoa_cycle:
        if (j + 32 <= n) {
        ...
        diff_j_vec = _mathkong_substract_bf16x32_genoa(a_j_vec, b_j_vec);
        cdiff_j_vec = _mm512_dpbf16_ps(cdiff_j_vec, (__m512bh)(diff_j_vec), (__m512bh)(c_vec));
        j += 32;
        if (j < n) goto mathkong_mahalanobis_bf16_genoa_cycle;
        sum_vec = _mm512_fmadd_ps(diff_i_vec, cdiff_j_vec, sum_vec);
    }
...
SIMSIMD_PUBLIC void mathkong_bilinear_bf16c_genoa(mathkong_bf16c_t const *a, mathkong_bf16c_t const *b,
    ...
    mathkong_bilinear_bf16c_skylake_cycle:
        if (j + 16 <= n) {
            b_vec = _mm512_loadu_epi16((mathkong_i16_t const *)(b + j));
            c_vec = _mm512_loadu_epi16((mathkong_i16_t const *)(c + i * n + j));
        }
        else {
            b_vec = _mm512_maskz_loadu_epi16(tail_mask, (mathkong_i16_t const *)(b + tail_start));
            c_vec = _mm512_maskz_loadu_epi16(tail_mask, (mathkong_i16_t const *)(c + i * n + tail_start));
        }
        ...
        cb_j_imag_vec = _mm512_dpbf16_ps(                              //
            ...
            (__m512bh)b_vec);
        j += 16;
        if (j < n) goto mathkong_bilinear_bf16c_skylake_cycle;
        // Horizontal sums are the expensive part of the computation:
        mathkong_f64_t const cb_j_real = _mathkong_reduce_f32x16_skylake(cb_j_real_vec);
        mathkong_f64_t const cb_j_imag = _mathkong_reduce_f32x16_skylake(cb_j_imag_vec);
        sum_real += a_i_real * cb_j_real - a_i_imag * cb_j_imag;
        sum_imag += a_i_real * cb_j_imag + a_i_imag * cb_j_real;
    }
...
SIMSIMD_PUBLIC void mathkong_bilinear_f16_sapphire_under32unrolled(mathkong_f16_t const *a, mathkong_f16_t const *b,
    ...
    for (; i + 4 <= n; i += 4) {
        ...
        // to avoid implicit casts from out `mathkong_f16_t` to `_Float16`.
        cb_j1 = _mm512_fmadd_ph(
            _mm512_castsi512_ph(_mm512_maskz_loadu_epi16(mask, c + n * (i + 0))),
            _mm512_mul_ph(b_vec, _mm512_castsi512_ph(_mm512_set1_epi16(((mathkong_i16_t const *)a)[i + 0]))), cb_j1);
        cb_j2 = _mm512_fmadd_ph(
            _mm512_castsi512_ph(_mm512_maskz_loadu_epi16(mask, c + n * (i + 1))),
            _mm512_mul_ph(b_vec, _mm512_castsi512_ph(_mm512_set1_epi16(((mathkong_i16_t const *)a)[i + 1]))), cb_j2);
        cb_j3 = _mm512_fmadd_ph(
            _mm512_castsi512_ph(_mm512_maskz_loadu_epi16(mask, c + n * (i + 2))),
            _mm512_mul_ph(b_vec, _mm512_castsi512_ph(_mm512_set1_epi16(((mathkong_i16_t const *)a)[i + 2]))), cb_j3);
        cb_j4 = _mm512_fmadd_ph(
            _mm512_castsi512_ph(_mm512_maskz_loadu_epi16(mask, c + n * (i + 3))),
            _mm512_mul_ph(b_vec, _mm512_castsi512_ph(_mm512_set1_epi16(((mathkong_i16_t const *)a)[i + 3]))), cb_j4);
    }
...
SIMSIMD_PUBLIC void mathkong_bilinear_f16_sapphire(mathkong_f16_t const *a, mathkong_f16_t const *b,
    ...
    if (n <= 32) {
        mathkong_bilinear_f16_sapphire_under32unrolled(a, b, c, n, result);
        return;
    }
    ...
    mathkong_bilinear_f16_sapphire_cycle:
        if (j + 32 <= n) {
            b_vec = _mm512_loadu_epi16(b + j);
            c_vec = _mm512_loadu_epi16(c + i * n + j);
        }
        ...
        j += 32;
        if (j < n) goto mathkong_bilinear_f16_sapphire_cycle;
...
SIMSIMD_PUBLIC void mathkong_mahalanobis_f16_sapphire(mathkong_f16_t const *a, mathkong_f16_t const *b,
    ...
    for (mathkong_size_t i = 0; i != n; ++i) {
        ...
        // The nested loop is cleaner to implement with a `goto` in this case:
    mathkong_mahalanobis_f16_sapphire_cycle:
        if (j + 32 <= n) {
            a_j_vec = _mm512_loadu_epi16(a + j);
            b_j_vec = _mm512_loadu_epi16(b + j);
            c_vec = _mm512_loadu_epi16(c + i * n + j);
        ...
        j += 32;
        if (j < n) goto mathkong_mahalanobis_f16_sapphire_cycle;
...
SIMSIMD_PUBLIC void mathkong_bilinear_f16c_sapphire(mathkong_f16c_t const *a, mathkong_f16c_t const *b,
    ...
    mathkong_bilinear_f16c_skylake_cycle:
        if (j + 16 <= n) {
            b_vec = _mm512_loadu_epi16((mathkong_i16_t const *)(b + j));
            c_vec = _mm512_loadu_epi16((mathkong_i16_t const *)(c + i * n + j));
        }
        ...
        j += 16;
        if (j < n) goto mathkong_bilinear_f16c_skylake_cycle;
        // Horizontal sums are the expensive part of the computation:
        mathkong_f32_t const cb_j_real = _mm512_reduce_add_ph(cb_j_real_vec);
        mathkong_f32_t const cb_j_imag = _mm512_reduce_add_ph(cb_j_imag_vec);
        sum_real += a_i_real * cb_j_real - a_i_imag * cb_j_imag;
        sum_imag += a_i_real * cb_j_imag + a_i_imag * cb_j_real;
    }
