...
#define MATHKONG_MAKE_INTERSECT_GALLOPING(name, input_type, counter_type)                                       \
    ...
    MATHKONG_PUBLIC void mathkong_intersect_##input_type##_##name(                                              \
        ...
        if (longer_length < shorter_length) {                                                                   \
            mathkong_##input_type##_t const *temp = shorter;                                                    \
            shorter = longer;                                                                                   \
            longer = temp;                                                                                      \
            mathkong_size_t temp_length = shorter_length;                                                       \
        ...
        if (longer_length < 64 * shorter_length) {                                                              \
            mathkong_intersect_##input_type##_accurate(shorter, longer, shorter_length, longer_length, result); \
            return;                                                                                             \
        }                                                                                                       \
                                                                                                                \
        ...
        mathkong_##counter_type##_t intersection_size = 0;                                                      \
        mathkong_size_t j = 0;                                                                                  \
        for (mathkong_size_t i = 0; i < shorter_length; ++i) {                                                  \
            mathkong_##input_type##_t shorter_i = shorter[i];                                                   \
            j = mathkong_galloping_search_##input_type(longer, j, longer_length, shorter_i);                    \
...
MATHKONG_INTERNAL mathkong_u32_t _mathkong_intersect_u16x32_ice(__m512i a, __m512i b) {
    ...
    return ~(mathkong_u32_t)(nm70 & _mathkong_u32_rol(&nm71, 8) & _mathkong_u32_rol(&nm72, 16) &
                             _mathkong_u32_ror(&nm73, 8));
}
...
MATHKONG_INTERNAL mathkong_u16_t _mathkong_intersect_u32x16_ice(__m512i a, __m512i b) {
    ...
    return ~(mathkong_u16_t)(nm0 & _mathkong_u16_rol(&nm1, 4) & _mathkong_u16_rol(&nm2, 8) &
...
MATHKONG_PUBLIC void mathkong_intersect_u16_ice(        //
    ...
    if (a_length < 64 && b_length < 64) {
        mathkong_intersect_u16_serial(a, b, a_length, b_length, results);
        return;
    }

    mathkong_u16_t const *const a_end = a + a_length;
    mathkong_u16_t const *const b_end = b + b_length;
    mathkong_size_t c = 0;
    union vec_t {
        __m512i zmm;
    ...
    while (a + 32 < a_end && b + 32 < b_end) {
        ...
        // Intersecting registers with `_mathkong_intersect_u16x32_ice` involves a lot of shuffling
        // and comparisons, so we want to avoid it if the slices don't overlap at all..
        mathkong_u16_t a_min;
        mathkong_u16_t a_max = a_vec.u16[31];
        mathkong_u16_t b_min = b_vec.u16[0];
        mathkong_u16_t b_max = b_vec.u16[31];
        ...
        a += 32 - _lzcnt_u32((mathkong_u32_t)a_step_mask);
        b += 32 - _lzcnt_u32((mathkong_u32_t)b_step_mask);

        // Now we are likely to have some overlap, so we can intersect the registers
        __mmask32 a_matches = _mathkong_intersect_u16x32_ice(a_vec.zmm, b_vec.zmm);
    ...
    mathkong_intersect_u16_serial(a, b, a_end - a, b_end - b, results);
    *results += c;
}
...
MATHKONG_PUBLIC void mathkong_intersect_u32_ice(        //
    ...
    if (a_length < 32 && b_length < 32) {
        mathkong_intersect_u32_serial(a, b, a_length, b_length, results);
        return;
    }

    mathkong_u32_t const *const a_end = a + a_length;
    mathkong_u32_t const *const b_end = b + b_length;
    mathkong_size_t c = 0;
    union vec_t {
        __m512i zmm;
        mathkong_u32_t u32[16];
        mathkong_u8_t u8[64];
    } a_vec, b_vec;
    ...
    while (a + 16 < a_end && b + 16 < b_end) {
        ...
        // Intersecting registers with `_mathkong_intersect_u32x16_ice` involves a lot of shuffling
        // and comparisons, so we want to avoid it if the slices don't overlap at all..
        mathkong_u32_t a_min;
        mathkong_u32_t a_max = a_vec.u32[15];
        mathkong_u32_t b_min = b_vec.u32[0];
        mathkong_u32_t b_max = b_vec.u32[15];
        ...
        // Now we are likely to have some overlap, so we can intersect the registers
        __mmask16 a_matches = _mathkong_intersect_u32x16_ice(a_vec.zmm, b_vec.zmm);
    ...
    mathkong_intersect_u32_serial(a, b, a_end - a, b_end - b, results);
    *results += c;
}
...
MATHKONG_PUBLIC void mathkong_intersect_u16_turin(      //
    ...
    while (a + 16 < a_end && b + 16 < b_end) {
        ...
        // and comparisons, so we want to avoid it if the slices don't overlap at all..
        mathkong_u16_t a_min;
        mathkong_u16_t a_max = a_vec.u16[15];
        mathkong_u16_t b_min = b_vec.u16[0];
        mathkong_u16_t b_max = b_vec.u16[15];
        ...
        a += 32 - _lzcnt_u32((mathkong_u32_t)a_step_mask); //? Is this correct? Needs testing!
        b += 32 - _lzcnt_u32((mathkong_u32_t)b_step_mask);
    }
...
MATHKONG_PUBLIC void mathkong_spdot_weights_u16_turin(                  //
    ...
    // The baseline implementation for very small arrays (2 registers or less) can be quite simple:
    if (a_length < 64 && b_length < 64) {
        mathkong_spdot_weights_u16_serial(a, b, a_weights, b_weights, a_length, b_length, results);
        return;
    }
    ...
    mathkong_u16_t const *const a_end = a + a_length;
    mathkong_u16_t const *const b_end = b + b_length;
    mathkong_size_t intersection_size = 0;
    union vec_t {
        __m256i ymm;
        __m256 ymmps;
        mathkong_u16_t u16[16];
    ...
    while (a + 16 < a_end && b + 16 < b_end) {
        ...
        __mmask16 a_step_mask = _mm256_cmple_epu16_mask(a_vec.ymm, b_last_broadcasted);
        __mmask16 b_step_mask = _mm256_cmple_epu16_mask(b_vec.ymm, a_last_broadcasted);
        int a_step = 32 - _lzcnt_u32((mathkong_u32_t)a_step_mask); //? Is this correct? Needs testing!
        int b_step = 32 - _lzcnt_u32((mathkong_u32_t)b_step_mask);
        a += a_step, a_weights += a_step;
        b += b_step, b_weights += b_step;
    }
    mathkong_spdot_weights_u16_serial(a, b, a_weights, b_weights, a_end - a, b_end - b, results);
    results[0] += intersection_size;
...
MATHKONG_PUBLIC void mathkong_spdot_counts_u16_turin(                 //
    ...
    // The baseline implementation for very small arrays (2 registers or less) can be quite simple:
    if (a_length < 64 && b_length < 64) {
        mathkong_spdot_counts_u16_serial(a, b, a_weights, b_weights, a_length, b_length, results);
        return;
    }
    ...
    mathkong_spdot_counts_u16_serial(a, b, a_weights, b_weights, a_end - a, b_end - b, results);
    results[0] += intersection_size;
...
MATHKONG_PUBLIC void mathkong_intersect_u16_neon(       //
    ...
    while (a + 8 < a_end && b + 8 < b_end) {
        ...
        // Intersecting registers with `_mathkong_intersect_u16x8_neon` involves a lot of shuffling
        // and comparisons, so we want to avoid it if the slices don't overlap at all..
        mathkong_u16_t a_min;
        mathkong_u16_t a_max = a_vec.u16[7];
        mathkong_u16_t b_min = b_vec.u16[0];
        mathkong_u16_t b_max = b_vec.u16[7];
        ...
        //
        //      mathkong_u64_t a_matches = __builtin_popcountll(
        //          _mathkong_u8_to_u4_neon(vreinterpretq_u8_u16(
        //              _mathkong_intersect_u16x8_neon(a_vec.u16x8, b_vec.u16x8))));
        //      c += a_matches / 8;
        //
        ...
        uint16x8_t a_matches = _mathkong_intersect_u16x8_neon(a_vec.u16x8, b_vec.u16x8);
        ...
        mathkong_u64_t a_step = _mathkong_clz_u64(_mathkong_u8_to_u4_neon( //
            vreinterpretq_u8_u16(vcleq_u16(a_vec.u16x8, b_last_broadcasted))));
        mathkong_u64_t b_step = _mathkong_clz_u64(_mathkong_u8_to_u4_neon( //
...
MATHKONG_PUBLIC void mathkong_intersect_u32_neon(       //
    ...
    while (a + 4 < a_end && b + 4 < b_end) {
        ...
        // Intersecting registers with `_mathkong_intersect_u32x4_neon` involves a lot of shuffling
        // and comparisons, so we want to avoid it if the slices don't overlap at all..
        mathkong_u32_t a_min;
        mathkong_u32_t a_max = a_vec.u32[3];
        mathkong_u32_t b_min = b_vec.u32[0];
        mathkong_u32_t b_max = b_vec.u32[3];
        ...
        //
        //     mathkong_u64_t a_matches = __builtin_popcountll(
        //         _mathkong_u8_to_u4_neon(vreinterpretq_u8_u32(
        //             _mathkong_intersect_u32x4_neon(a_vec.u32x4, b_vec.u32x4))));
        //     c += a_matches / 16;
        //
        // Alternatively, we can we can transform match-masks into "ones", accumulate them between the cycles,
        // and merge all together in the end.
        uint32x4_t a_matches = _mathkong_intersect_u32x4_neon(a_vec.u32x4, b_vec.u32x4);
...
MATHKONG_PUBLIC void mathkong_intersect_u16_sve2(     //
    ...
    mathkong_size_t const register_size = svcnth();
    mathkong_size_t const lanes_count = register_size / 8;
    mathkong_size_t a_idx = 0, b_idx = 0;
    mathkong_size_t c = 0;
    ...
    while (a_idx < a_length && b_idx < b_length) {
        ...
        // and comparisons, so we want to avoid it if the slices don't overlap at all..
        mathkong_u16_t a_min;
        mathkong_u16_t a_max = svlastb(a_progress, a_vec);
        mathkong_u16_t b_min = svlasta(svpfalse_b(), b_vec);
        mathkong_u16_t b_max = svlastb(b_progress, b_vec);
        ...
        mathkong_u64_t a_step = svcntp_b16(a_progress, a_mask);
        mathkong_u64_t b_step = svcntp_b16(b_progress, b_mask);

        // Compare `a_vec` with each lane of `b_vec`
        svbool_t equal_mask = svmatch_u16(a_progress, a_vec, b_vec);
        for (mathkong_size_t i = 1; i < lanes_count; i++) {
            b_vec = svext_u16(b_vec, b_vec, 8);
            equal_mask = svorr_z(svptrue_b16(), equal_mask, svmatch_u16(a_progress, a_vec, b_vec));
        }
        mathkong_size_t equal_count = svcntp_b16(svptrue_b16(), equal_mask);
...
MATHKONG_PUBLIC void mathkong_intersect_u32_sve2(mathkong_u32_t const *a, mathkong_u32_t const *b,
    ...
    mathkong_size_t const lanes_count = register_size / 4;
    mathkong_size_t a_idx = 0, b_idx = 0;
    mathkong_size_t c = 0;
    ...
    while (a_idx < a_length && b_idx < b_length) {
        ...
        // and comparisons, so we want to avoid it if the slices don't overlap at all..
        mathkong_u32_t a_min;
        mathkong_u32_t a_max = svlastb(a_progress, a_vec);
        mathkong_u32_t b_min = svlasta(svpfalse_b(), b_vec);
        mathkong_u32_t b_max = svlastb(b_progress, b_vec);
        ...
        mathkong_u64_t a_step = svcntp_b32(a_progress, a_mask);
        mathkong_u64_t b_step = svcntp_b32(b_progress, b_mask);

        // Comparing `a_vec` with each lane of `b_vec` can't be done with `svmatch`,
        // the same way as in `mathkong_intersect_u16_sve2`, as that instruction is only
        // available for 8-bit and 16-bit integers.
        //
        //      svbool_t equal_mask = svpfalse_b();
        //      for (mathkong_size_t i = 0; i < register_size; i++) {
        //          equal_mask = svorr_z(svptrue_b32(), equal_mask, svcmpeq_u32(a_progress, a_vec, b_vec));
        //          b_vec = svext_u32(b_vec, b_vec, 1);
        //      }
        //      mathkong_size_t equal_count = svcntp_b32(a_progress, equal_mask);
        //
...
MATHKONG_PUBLIC void mathkong_spdot_counts_u16_sve2(                  //
    ...
    while (a_idx < a_length && b_idx < b_length) {
        ...
        for (mathkong_size_t i = 0; i < lanes_count; i++) {
            svbool_t equal_mask = svmatch_u16(a_progress, a_vec, b_vec);
    ...
    results[0] = (mathkong_distance_t)intersection_size;
