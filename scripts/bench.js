...
const mathkong = require("../javascript/dist/cjs/mathkong.js");
...
const mathm1 = math.matrix(
  Array.from({ length: dimensions }, () => Math.random() * 100)
);
const mathm2 = math.matrix(
  Array.from({ length: dimensions }, () => Math.random() * 100)
);
...
singleSuite
  ...
  // MathKong
  .add("TypedArray of Float32 with MathKong", () => {
    mathkong.angular(floatArray1, floatArray2);
  })
  .add("TypedArray of Int8 with MathKong", () => {
    mathkong.angular(intArray1, intArray2);
  })
  ...
  .on("complete", () => {
    console.log(
      "Fastest Single-Vector Processing is " +
        singleSuite.filter("fastest").map("name")
    );
  })
  .run({
    noCache: true,
...
batchSuite
  ...
  .on("complete", () => {
    console.log(
      "Fastest Batch-Vector Processing is " +
        batchSuite.filter("fastest").map("name")
    );
  })
  .run({
    noCache: true,
