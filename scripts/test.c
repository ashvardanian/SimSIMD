...
void test_utilities(void) {
    mathkong_capability_t capabilities = mathkong_capabilities();
    ...
    int uses_turin = mathkong_uses_turin();
    int uses_sierra = mathkong_uses_sierra();

    assert(uses_neon == ((capabilities & mathkong_cap_neon_k) != 0));
    assert(uses_sve == ((capabilities & mathkong_cap_sve_k) != 0));
    assert(uses_haswell == ((capabilities & mathkong_cap_haswell_k) != 0));
    assert(uses_skylake == ((capabilities & mathkong_cap_skylake_k) != 0));
    assert(uses_ice == ((capabilities & mathkong_cap_ice_k) != 0));
    assert(uses_genoa == ((capabilities & mathkong_cap_genoa_k) != 0));
    assert(uses_sapphire == ((capabilities & mathkong_cap_sapphire_k) != 0));
    assert(uses_turin == ((capabilities & mathkong_cap_turin_k) != 0));
    assert(uses_sierra == ((capabilities & mathkong_cap_sierra_k) != 0));
}
...
void test_saturating_arithmetic(void) {
    ...
    mathkong_u8_t u8_a = 200, u8_b = 100, u8_r;
    _mathkong_u8_sadd(&u8_a, &u8_b, &u8_r);
    assert(u8_r == 255); // Overflow case for u8

    mathkong_i8_t i8_a = 100, i8_b = 40, i8_r;
    _mathkong_i8_sadd(&i8_a, &i8_b, &i8_r);
    assert(i8_r == 127); // Overflow case for i8

    mathkong_i8_t i8_under_a = -100, i8_under_b = -40;
    _mathkong_i8_sadd(&i8_under_a, &i8_under_b, &i8_r);
    assert(i8_r == -128); // Underflow case for i8

    mathkong_u16_t u16_a = 50000, u16_b = 20000, u16_r;
    _mathkong_u16_sadd(&u16_a, &u16_b, &u16_r);
    assert(u16_r == 65535); // Overflow case for u16

    mathkong_i16_t i16_a = 30000, i16_b = 10000, i16_r;
    _mathkong_i16_sadd(&i16_a, &i16_b, &i16_r);
    assert(i16_r == 32767); // Overflow case for i16

    mathkong_i16_t i16_under_a = -20000, i16_under_b = -15000;
    _mathkong_i16_sadd(&i16_under_a, &i16_under_b, &i16_r);
    assert(i16_r == -32768); // Underflow case for i16

    // Test cases for multiplication functions
    mathkong_u8_t u8_m_a = 20, u8_m_b = 20, u8_m_r;
    _mathkong_u8_smul(&u8_m_a, &u8_m_b, &u8_m_r);
    assert(u8_m_r == 255); // Overflow case for u8 multiplication

    mathkong_i8_t i8_m_a = 10, i8_m_b = -13, i8_m_r;
    _mathkong_i8_smul(&i8_m_a, &i8_m_b, &i8_m_r);
    assert(i8_m_r == -128); // Underflow case for i8 multiplication

    mathkong_i8_t i8_m_under_a = -100, i8_m_under_b = 2;
    _mathkong_i8_smul(&i8_m_under_a, &i8_m_under_b, &i8_m_r);
    assert(i8_m_r == -128); // Underflow case for i8 multiplication

    mathkong_u16_t u16_m_a = 300, u16_m_b = 300, u16_m_r;
    _mathkong_u16_smul(&u16_m_a, &u16_m_b, &u16_m_r);
    assert(u16_m_r == 65535); // Overflow case for u16 multiplication

    mathkong_i16_t i16_m_a = 200, i16_m_b = 300, i16_m_r;
    _mathkong_i16_smul(&i16_m_a, &i16_m_b, &i16_m_r);
    assert(i16_m_r == 32767); // Overflow case for i16 multiplication

    mathkong_i16_t i16_m_under_a = -200, i16_m_under_b = 300;
    _mathkong_i16_smul(&i16_m_under_a, &i16_m_under_b, &i16_m_r);
    assert(i16_m_r == -32768); // Underflow case for i16 multiplication

    // Normal cases without overflow
    mathkong_u8_t u8_n_a = 20, u8_n_b = 15, u8_n_r;
    _mathkong_u8_sadd(&u8_n_a, &u8_n_b, &u8_n_r);
    assert(u8_n_r == 35);

    mathkong_i8_t i8_n_a = -10, i8_n_b = 20, i8_n_r;
    _mathkong_i8_sadd(&i8_n_a, &i8_n_b, &i8_n_r);
    assert(i8_n_r == 10);

    // Floating-point cases
    mathkong_f32_t f32_a = 1.5f, f32_b = 2.5f, f32_r;
    _mathkong_f32_sadd(&f32_a, &f32_b, &f32_r);
    assert(f32_r == 4.0f); // Normal addition for f32

    mathkong_f32_t f32_m_a = 1.5f, f32_m_b = 2.0f;
    _mathkong_f32_smul(&f32_m_a, &f32_m_b, &f32_r);
...
void test_xd_index(void) {
    ...
    // 1D array
    shape[0] = 10;
    strides[0] = 1 * sizeof(mathkong_u8_t);
    mathkong_xd_index_init(&xd_index);
    for (mathkong_size_t i = 0; i < 10; i++) {
        assert(xd_index.byte_offset == i * sizeof(mathkong_u8_t));
        assert(xd_index.coordinates[0] == i);
        assert(mathkong_xd_index_linearize(shape, strides, 1, &xd_index.coordinates[0], &linear_byte_offset));
        assert(linear_byte_offset == i * sizeof(mathkong_u8_t));
        assert(mathkong_xd_index_next(shape, strides, 1, &xd_index.coordinates[0], &xd_index.byte_offset) == (i < 9));
    }

    // 2D array
    shape[0] = 10, shape[1] = 5;
    strides[0] = 5 * sizeof(mathkong_u8_t), strides[1] = 1 * sizeof(mathkong_u8_t);
    mathkong_xd_index_init(&xd_index);
    for (mathkong_size_t i = 0; i < 10; i++) {
        for (mathkong_size_t j = 0; j < 5; j++) {
            assert(xd_index.byte_offset == (i * 5 + j) * sizeof(mathkong_u8_t));
            assert(xd_index.coordinates[0] == i);
            assert(xd_index.coordinates[1] == j);
            assert(mathkong_xd_index_linearize(shape, strides, 2, &xd_index.coordinates[0], &linear_byte_offset));
            assert(linear_byte_offset == (i * 5 + j) * sizeof(mathkong_u8_t));
            assert(mathkong_xd_index_next(shape, strides, 2, &xd_index.coordinates[0], &xd_index.byte_offset) ==
                   (i != 9 || j != 4));
        }
    }
    ...
    shape[0] = 10, shape[1] = 5;
    strides[0] = 10 * sizeof(mathkong_u8_t), strides[1] = 2 * sizeof(mathkong_u8_t);
    mathkong_xd_index_init(&xd_index);
    for (mathkong_size_t i = 0; i < 10; i++) {
        for (mathkong_size_t j = 0; j < 5; j++) {
            assert(xd_index.byte_offset == (i * 5 + j) * 2 * sizeof(mathkong_u8_t));
            assert(xd_index.coordinates[0] == i);
            assert(xd_index.coordinates[1] == j);
            assert(mathkong_xd_index_linearize(shape, strides, 2, &xd_index.coordinates[0], &linear_byte_offset));
            assert(linear_byte_offset == (i * 5 + j) * 2 * sizeof(mathkong_u8_t));
            assert(mathkong_xd_index_next(shape, strides, 2, &xd_index.coordinates[0], &xd_index.byte_offset) ==
                   (i != 9 || j != 4));
        }
    }
    ...
    strides[0] = 41 * sizeof(mathkong_u8_t), strides[1] = 7 * sizeof(mathkong_u8_t),
    strides[2] = 2 * sizeof(mathkong_u8_t);
    mathkong_xd_index_init(&xd_index);
    for (mathkong_size_t i = 0; i < 10; i++) {
        for (mathkong_size_t j = 0; j < 5; j++) {
            for (mathkong_size_t k = 0; k < 3; k++) {
                assert(xd_index.byte_offset == (i * strides[0] + j * strides[1] + k * strides[2]));
                assert(xd_index.coordinates[0] == i);
                assert(xd_index.coordinates[1] == j);
                assert(xd_index.coordinates[2] == k);
                assert(mathkong_xd_index_linearize(shape, strides, 3, &xd_index.coordinates[0], &linear_byte_offset));
                assert(linear_byte_offset == (i * strides[0] + j * strides[1] + k * strides[2]));
                assert(mathkong_xd_index_next(shape, strides, 3, &xd_index.coordinates[0], &xd_index.byte_offset) ==
                       (i != 9 || j != 4 || k != 2));
            }
        }
    }
    ...
    {
        mathkong_f32_t tensor[11][43][7];
        // Fill tensor with values
        for (mathkong_size_t i = 0; i < 11; i++) {
            for (mathkong_size_t j = 0; j < 43; j++)
                for (mathkong_size_t k = 0; k < 7; k++) tensor[i][j][k] = i * 10000 + j * 100 + k * 1;
        }
        // Accumulate a slice: tensor[1:9:2, 2:42:4, 1:5:3] ~ 4 channels, 10 rows, 2 columns
        mathkong_xd_index_init(&xd_index);
        shape[0] = _mathkong_divide_ceil(9 - 1, 2);
        shape[1] = _mathkong_divide_ceil(42 - 2, 4);
        shape[2] = _mathkong_divide_ceil(5 - 1, 3);
        strides[0] = 43 * 7 * sizeof(mathkong_f32_t) * 2; // Physical size of 2 channels
        strides[1] = 7 * sizeof(mathkong_f32_t) * 4;      // Physical size of 4 rows
        strides[2] = 3 * sizeof(mathkong_f32_t);          // Physical size of 3 columns
        // Accumulate using native indexing
        mathkong_f32_t sum_native = 0;
        for (mathkong_size_t i = 1; i < 9; i += 2) {
            for (mathkong_size_t j = 2; j < 42; j += 4) {
                for (mathkong_size_t k = 1; k < 5; k += 3) { //
                    sum_native += tensor[i][j][k];
                }
            }
        }
        // Accumulate using our `mathkong_xd_index_t` iterator
        mathkong_f32_t sum_with_xd_index = 0;
        mathkong_f32_t sum_native_running = 0;
        for (mathkong_size_t i = 1; i < 9; i += 2) {
            for (mathkong_size_t j = 2; j < 42; j += 4) {
                for (mathkong_size_t k = 1; k < 5; k += 3) {
                    mathkong_size_t const expected_global_offset = //
                        ((i - 1) / 2) * shape[1] * shape[2] +      //
                        ((j - 2) / 4) * shape[2] +                 //
                        ((k - 1) / 3);                             //
                    mathkong_f32_t const entry_native = tensor[i][j][k];
                    mathkong_f32_t const entry_from_byte_offset =
                        *(mathkong_f32_t *)_mathkong_advance_by_bytes(&tensor[1][2][1], xd_index.byte_offset);
                    mathkong_f32_t const entry_from_coordinate = tensor //
                    ...
                    mathkong_xd_index_next(shape, strides, 3, &xd_index.coordinates[0], &xd_index.byte_offset);
                }
            }
        }
    }
...
void test_approximate_math(void) {
    ...
    typedef struct error_aggregator {
        mathkong_f64_t absolute_error;
        mathkong_f64_t relative_error;
        mathkong_f64_t max_error;
    } error_aggregator;
    ...
    mathkong_f32_t const range_min = -3.14159265358979323846f * 2;
    mathkong_f32_t const range_max = 3.14159265358979323846f * 2;

    // Test all possible values of f32 within ranges: [-π, -1], [-1, -0], [0, 1], [1, π].
    mathkong_size_t const step = 1;
    mathkong_size_t const count_tests = 0xFFFFFFFFull / step;
    union {
        mathkong_f32_t f32;
        mathkong_u32_t u32;
    } x;
    ...
    for (x.u32 = 0; (x.u32 + step) <= 0xFFFFFFFFull; x.u32 += step) {
        ...
        mathkong_f32_t f32_sin_baseline = sinf(x.f32);
        mathkong_f32_t f32_sin_approx = mathkong_f32_sin(x.f32);
        mathkong_f32_t f32_sin_diff = fabsf(f32_sin_baseline - f32_sin_approx);
        mathkong_f32_t f32_sin_max = fmaxf(fabsf(f32_sin_baseline), fabsf(f32_sin_approx));
    ...
    for (x.u32 = 0; (x.u32 + step) <= 0xFFFFFFFFull; x.u32 += step) {
        if (x.f32 < range_min || x.f32 > range_max) continue;
        mathkong_f32_t f32_cos_baseline = cosf(x.f32);
        mathkong_f32_t f32_cos_approx = mathkong_f32_cos(x.f32);
        mathkong_f32_t f32_cos_diff = fabsf(f32_cos_baseline - f32_cos_approx);
        mathkong_f32_t f32_cos_max = fmaxf(fabsf(f32_cos_baseline), fabsf(f32_cos_approx));
    ...
    for (x.u32 = 0; (x.u32 + step) <= 0xFFFFFFFFull; x.u32 += step) {
        if (x.f32 < range_min || x.f32 > range_max) continue;
        mathkong_f32_t f32_atan_baseline = atanf(x.f32);
        mathkong_f32_t f32_atan_approx = mathkong_f32_atan(x.f32);
        mathkong_f32_t f32_atan_diff = fabs(f32_atan_baseline - f32_atan_approx);
        mathkong_f32_t f32_atan_max = fmax(fabs(f32_atan_baseline), fabs(f32_atan_approx));
    ...
    for (x.u32 = 0; (x.u32 + step) <= 0xFFFFFFFFull; x.u32 += step) {
        if (x.f32 < range_min || x.f32 > range_max) continue;
        mathkong_f32_t f32_atan2_baseline = atan2f(x.f32, (1 - x.f32));
        mathkong_f32_t f32_atan2_approx = mathkong_f32_atan2(x.f32, (1 - x.f32));
        mathkong_f32_t f32_atan2_diff = fabs(f32_atan2_baseline - f32_atan2_approx);
        mathkong_f32_t f32_atan2_max = fmax(fabs(f32_atan2_baseline), fabs(f32_atan2_approx));
    ...
    for (x.u32 = 0; (x.u32 + step) <= 0xFFFFFFFFull; x.u32 += step) {
        if (x.f32 < range_min || x.f32 > range_max) continue;
        mathkong_f64_t f64_sin_baseline = sin(x.f32);
        mathkong_f64_t f64_sin_approx = mathkong_f64_sin(x.f32);
        mathkong_f64_t f64_sin_diff = fabs(f64_sin_baseline - f64_sin_approx);
        mathkong_f64_t f64_sin_max = fmax(fabs(f64_sin_baseline), fabs(f64_sin_approx));
    ...
    for (x.u32 = 0; (x.u32 + step) <= 0xFFFFFFFFull; x.u32 += step) {
        if (x.f32 < range_min || x.f32 > range_max) continue;
        mathkong_f64_t f64_cos_baseline = cos(x.f32);
        mathkong_f64_t f64_cos_approx = mathkong_f64_cos(x.f32);
        mathkong_f64_t f64_cos_diff = fabs(f64_cos_baseline - f64_cos_approx);
        mathkong_f64_t f64_cos_max = fmax(fabs(f64_cos_baseline), fabs(f64_cos_approx));
    ...
    for (x.u32 = 0; (x.u32 + step) <= 0xFFFFFFFFull; x.u32 += step) {
        if (x.f32 < range_min || x.f32 > range_max) continue;
        mathkong_f64_t f64_atan_baseline = atan(x.f32);
        mathkong_f64_t f64_atan_approx = mathkong_f64_atan(x.f32);
        mathkong_f64_t f64_atan_diff = fabs(f64_atan_baseline - f64_atan_approx);
        mathkong_f64_t f64_atan_max = fmax(fabs(f64_atan_baseline), fabs(f64_atan_approx));
    ...
    for (x.u32 = 0; (x.u32 + step) <= 0xFFFFFFFFull; x.u32 += step) {
        if (x.f32 < range_min || x.f32 > range_max) continue;
        mathkong_f64_t f64_atan2_baseline = atan2(x.f32, (1 - x.f32));
        mathkong_f64_t f64_atan2_approx = mathkong_f64_atan2(x.f32, (1 - x.f32));
        mathkong_f64_t f64_atan2_diff = fabs(f64_atan2_baseline - f64_atan2_approx);
        mathkong_f64_t f64_atan2_max = fmax(fabs(f64_atan2_baseline), fabs(f64_atan2_approx));
...
void test_distance_from_itself(void) {
    ...
    mathkong_f64c_t f64cs[768];
    mathkong_f32c_t f32cs[768];
    mathkong_f16c_t f16cs[768];
    mathkong_bf16c_t bf16cs[768];

    mathkong_i8_t i8s[1536];
    mathkong_u8_t u8s[1536];
    mathkong_b8_t b8s[1536 / 8];     // 8 bits per word
    mathkong_distance_t distance[2]; // For complex dot-products we need two values

    // Cosine distance between two vectors
    mathkong_angular_i8(i8s, i8s, 1536, &distance[0]);
    mathkong_angular_u8(u8s, u8s, 1536, &distance[0]);
    mathkong_angular_f16(f16s, f16s, 1536, &distance[0]);
    mathkong_angular_bf16(bf16s, bf16s, 1536, &distance[0]);
    mathkong_angular_f32(f32s, f32s, 1536, &distance[0]);
    mathkong_angular_f64(f64s, f64s, 1536, &distance[0]);

    // Euclidean distance between two vectors
    mathkong_l2sq_i8(i8s, i8s, 1536, &distance[0]);
    mathkong_l2sq_u8(u8s, u8s, 1536, &distance[0]);
    mathkong_l2sq_f16(f16s, f16s, 1536, &distance[0]);
    mathkong_l2sq_bf16(bf16s, bf16s, 1536, &distance[0]);
    mathkong_l2sq_f32(f32s, f32s, 1536, &distance[0]);
    mathkong_l2sq_f64(f64s, f64s, 1536, &distance[0]);

    // Inner product between two vectors
    mathkong_dot_i8(i8s, i8s, 1536, &distance[0]);
    mathkong_dot_u8(u8s, u8s, 1536, &distance[0]);
    mathkong_dot_f16(f16s, f16s, 1536, &distance[0]);
    mathkong_dot_bf16(bf16s, bf16s, 1536, &distance[0]);
    mathkong_dot_f32(f32s, f32s, 1536, &distance[0]);
    mathkong_dot_f64(f64s, f64s, 1536, &distance[0]);

    // Complex inner product between two vectors
    mathkong_dot_bf16c(bf16cs, bf16cs, 768, &distance[0]);
    mathkong_dot_f16c(f16cs, f16cs, 768, &distance[0]);
    mathkong_dot_f32c(f32cs, f32cs, 768, &distance[0]);
    mathkong_dot_f64c(f64cs, f64cs, 768, &distance[0]);

    // Complex conjugate inner product between two vectors
    mathkong_vdot_bf16c(bf16cs, bf16cs, 768, &distance[0]);
    mathkong_vdot_f16c(f16cs, f16cs, 768, &distance[0]);
    mathkong_vdot_f32c(f32cs, f32cs, 768, &distance[0]);
    mathkong_vdot_f64c(f64cs, f64cs, 768, &distance[0]);

    // Hamming distance between two vectors
    mathkong_hamming_b8(b8s, b8s, 1536 / 8, &distance[0]);

    // Jaccard distance between two vectors
    mathkong_jaccard_b8(b8s, b8s, 1536 / 8, &distance[0]);

    // Jensen-Shannon divergence between two vectors
    mathkong_js_f16(f16s, f16s, 1536, &distance[0]);
    mathkong_js_bf16(bf16s, bf16s, 1536, &distance[0]);
    mathkong_js_f32(f32s, f32s, 1536, &distance[0]);
    mathkong_js_f64(f64s, f64s, 1536, &distance[0]);

    // Kullback-Leibler divergence between two vectors
    mathkong_kl_f16(f16s, f16s, 1536, &distance[0]);
    mathkong_kl_bf16(bf16s, bf16s, 1536, &distance[0]);
    mathkong_kl_f32(f32s, f32s, 1536, &distance[0]);
    mathkong_kl_f64(f64s, f64s, 1536, &distance[0]);
}
