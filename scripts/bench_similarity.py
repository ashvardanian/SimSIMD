...
This script benchmarks the performance of MathKong against other libraries,
...
# NumPy and MathKong are obligatory for benchmarking
import numpy as np
...
class Kernel:
    ...
    mathkong_func: callable
    mathkong_all_pairs_func: callable
    ...
    def for_dtypes(
        ...
        return [
            Kernel(
                ...
                mathkong_func=mathkong_func,
                mathkong_all_pairs_func=mathkong_all_pairs_func,
...
class Result:
    ...
    mathkong_seconds: Union[float, Exception]
...
def yield_batch_results(
    ...
    for kernel in kernels:
        ...
        mathkong_func = kernel.mathkong_func
        result = Result(
            kernel.dtype,
            kernel.name,
            baseline_seconds=0,
        ...
        # Try obtaining the MathKong measurements
        try:
            for i in range(1, count_matrices_per_dtype):
                result.mathkong_seconds += latency(
                    mathkong_func,
                    matrices_numpy[i - 1],
                    matrices_numpy[i],
                    count_repetitions_per_matrix,
...
def yield_all_pairs_results(
    ...
    for kernel in kernels:
        ...
        mathkong_func = kernel.mathkong_all_pairs_func
        result = Result(
            kernel.dtype,
            kernel.name,
            baseline_seconds=0,
...
def result_to_row(result: Result) -> List[str]:
    ...
    if isinstance(result.mathkong_seconds, float):
        ops_per_second = result.distance_calculations / result.mathkong_seconds
        gbs_per_second = result.bytes_per_vector * ops_per_second / 1e9
        mathkong_cell = f"{ops_per_second:,.0f} ops/s, {gbs_per_second:,.3f} GB/s"
    if isinstance(result.baseline_seconds, float) and isinstance(result.mathkong_seconds, float):
        improvement_cell = f"{result.baseline_seconds / result.mathkong_seconds:,.2f} x"

    return [dtype_cell, name_cell, baseline_cell, mathkong_cell, improvement_cell]


def main():
    # Argument parsing
    parser = argparse.ArgumentParser(description="Benchmark MathKong and other libraries")
    parser.add_argument(
        "--ndim",
        type=int,
        default=1536,
        help="""
            ...
            In case of MathKong, the inputs will be treated at the bit-level.
    ...
    try:
        ...
        # Log versions of MathKong, NumPy, SciPy, and scikit-learn
        print(f"- MathKong version: {simd.__version__}")
    ...
    columns_headers = ["Data Type", "Method", "Baseline", "MathKong", "Improvement"]
    results_rows = []
