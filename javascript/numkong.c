...
static nk_datatype_t kernel_output_dtype(nk_metric_kind_t kind, nk_datatype_t input) {
    switch (kind) {
    case nk_metric_dot_k: return nk_dot_output_datatype(input);
    case nk_metric_angular_k: return nk_angular_output_datatype(input);
    case nk_metric_sqeuclidean_k: return nk_l2sq_output_datatype(input);
    case nk_metric_euclidean_k: return nk_l2_output_datatype(input);
    default: return nk_f64_k;
    }
}

napi_value dense(napi_env env, napi_callback_info info, nk_metric_kind_t metric_kind, nk_datatype_t datatype) {
    size_t argc = 2;
    napi_value args[2];
    napi_status status;
    ...
    nk_find_kernel_punned(metric_kind, datatype, static_capabilities, nk_cap_any_k, (nk_kernel_punned_t *)&metric,
                          &capability);
    if (metric == NULL) {
    ...
    nk_datatype_t out_dtype = kernel_output_dtype(metric_kind, datatype);
...
napi_value api_ip(napi_env env, napi_callback_info info) {
    return dense(env, info, nk_metric_dot_k, nk_datatype_unknown_k);
}
napi_value api_angular(napi_env env, napi_callback_info info) {
    return dense(env, info, nk_metric_angular_k, nk_datatype_unknown_k);
}
napi_value api_l2sq(napi_env env, napi_callback_info info) {
    return dense(env, info, nk_metric_sqeuclidean_k, nk_datatype_unknown_k);
}
napi_value api_l2(napi_env env, napi_callback_info info) {
    return dense(env, info, nk_metric_euclidean_k, nk_datatype_unknown_k);
}
...
napi_value api_hamming(napi_env env, napi_callback_info info) { return dense(env, info, nk_metric_hamming_k, nk_b8_k); }
napi_value api_jaccard(napi_env env, napi_callback_info info) { return dense(env, info, nk_metric_jaccard_k, nk_b8_k); }
