...
try {
  ...
  compiled = fallback;
  console.warn(
    "It seems like your environment doesn't support the native mathkong module, so we are providing a JS fallback."
  );
}
...
/**
 * @brief Finds the directory where the native build of the mathkong module is located.
...
function getBuildDir(dir: string) {
  ...
  if (path.basename(dir) === ".next") {
    ...
    const sideways = path.join(dir, "..", "node_modules", "mathkong");
    if (existsSync(sideways)) return getBuildDir(sideways);
  }
  if (dir === "/") throw new Error("Could not find native build for mathkong");
